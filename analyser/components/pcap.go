...
func (p Pcap) Acc() *report.Accumulator {
	return &p.acc
}
...
func (p *Pcap) handleFile(file string) {
	...
	if r, err := pcapgo.NewReader(p.r); err != nil {
		...
		wg := &sync.WaitGroup{}
		packets := make(chan UnparsedMeasurement, 2000)
		chain := NewParser(&packets)
		go chain.Run()
		inputFilter := NewFilter(chain, InOutFilter{})
		go inputFilter.Run()
		counter := NewCounter(inputFilter, DayCounter{identifier: "ethIn"})
		go counter.Run()
		counterNo := NewCounterFromFilter(inputFilter, DayCounter{identifier: "ethOut"})
		go counterNo.Run()
		ep := NewEndpoint(counter, wg)
		go ep.Run()
		ep2 := NewEndpoint(counterNo, wg)
		go ep2.Run()
		for {
			...
			measurement := UnparsedMeasurement{&data, &ci}
			packets <- measurement
